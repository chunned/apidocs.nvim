  ...
  for i = #lines, 1, -1 do
    ...
    if m ~= nil and m:match("^file://") then
      ...
      elseif #file_id == 2 then
        ...
        local local_path, local_fname = fix_file_links_resolve_fname(choice, path_to_name, file_guessed_subpath_str,
          file_id[1]:gsub("^/", ""))
...
local function apidoc_install(choice, slugs_to_mtimes, cont)
  ...
  vim.system({ "curl", "-L", "https://documents.devdocs.io/" .. choice .. "/index.json?" .. mtime }, { text = true },
    vim.schedule_wrap(function(res)
      ...
      vim.system({ "curl", "-L", "https://documents.devdocs.io/" .. choice .. "/db.json?" .. mtime }, { text = true },
        vim.schedule_wrap(function(res)
          local data = vim.fn.json_decode(res.stdout)
      ...
      (#eq? @_name "id")
            ...
            for id, node, metadata in query:iter_captures(tree:root(), contents) do
              if node:next_named_sibling():named_child_count() > 0 then
                ...
                elseif node:parent() ~= nil and node:parent():parent() ~= nil
                  ...
                  name_and_id_to_string_nearby[sanitized_key][id_val] =
                      vim.treesitter.get_node_text(node:parent():parent():next_named_sibling():named_children()[2],
                        contents)
          ...
          for path, name in pairs(path_to_name) do
            ...
            if #file_id == 2 then
              local sanitized_containing_file_name = sanitize_fname((path_to_name[file_id[1]] or file_id[1]) ..
                "#" .. file_id[1])
              if name_and_id_to_pos[sanitized_containing_file_name] == nil then
                ...
                if path_to_type[file_id[1]] ~= nil then
                  file:write("<p>&gt; " ..
                    choice .. "/" .. path_to_type[file_id[1]] .. "/" .. path_to_name[file_id[1]] .. "\n</p>\n")
                else
                  file:write("<p>&gt; " .. choice .. "\n</p>\n")
                end
          ...
          vim.system({
            "sh", "-c",
            [[find . -maxdepth 1 -name '*.html' -print0 | xargs ]] ..
            sysspecificopts ..
            [[-0 -P 8 -I param sh -c "elinks -config-dir ]] ..
            data_folder .. [[ -dump 'param' > 'param'.md && rm 'param'"]]
          ...
          vim.notify("Starting initial postprocessing...")
          vim.system({ "rg", "-l", "│" }, { cwd = target_path }, vim.schedule_wrap(function(res)
            ...
            vim.notify("Applying final postprocessing...")
            ...
            vim.notify("Finished fetching documentation for " ..
              choice .. " in " .. elapsed .. "s. All parsing: " .. all_parsing
              ..
              "s. All reading IDs: " ..
              all_reading_ids ..
              "s. All writing: " ..
              elapsed_writing .. "s. All elinks: " .. elapsed_elinks .. "s. All post-process: " .. elapsed_pp .. "s.")
